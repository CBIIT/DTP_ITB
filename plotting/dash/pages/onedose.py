        ...
        dbc.Form(id='od-input-form', children=[
            dbc.Row(children=[
                html.Div(className='d-grid', children=[
                    ...
                    dbc.Select(id="od-expid-dropdown")
                ])
            ], style=ROW_PADDING),
            dbc.Row(children=[
                html.Div(className='d-grid', children=[
...
layout = dbc.Row(children=[
    ...
    dbc.Col(dbc.Card([
        ...
        dbc.CardBody(dcc.Loading(id='od-graph-content-loading',type='default',children=html.Div(html.P('Please select Exp Nbr, NSC'), id='od-graph-content', )))
    ]),
        width=10
    )
], style=ROW_PADDING)
