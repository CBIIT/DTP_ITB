...
def get_left_select(expid, nsc):
    ...
    if expid is None and nsc is None:
        expid_dropdown = dcc.Dropdown(id="s-expid-dropdown", placeholder="Select")
        nsc_dropdown = dcc.Dropdown(id="s-nsc-dropdown", disabled=True)
    else:
        expid_dropdown = dcc.Dropdown(id="s-expid-dropdown", value=expid, options=[{'label': expid, 'value': expid}])
        nsc_dropdown = dcc.Dropdown(id="s-nsc-dropdown", value=nsc, options=[{'label': nsc, 'value': nsc}])
...
def handle_selections(expid, nsc, data):
    ...
    if expid is not None:
        res = [d for d in dataService.INVIVO_COLL.aggregate(
            [
                {
                    '$match': {
                        'expid': expid
        ...
        for doc in res:
            nscs = nscs + doc['invivonsc']
        nscs = nscs.replace('\'', '').split(',')
...
def get_graphs(active_tab, group, nsc, expid):
    ...
    if (group is None) or (not ctx.triggered):
        ...
        if active_tab == 'summ-tab':
            return get_summary_components(expid, nsc)
